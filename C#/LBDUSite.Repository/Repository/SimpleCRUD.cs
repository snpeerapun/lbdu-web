...
namespace LBDUSite.Repository
{
    ...
    public static partial class SimpleCRUD
    {
        ...
        /// <param name="commandTimeout"></param>
        /// <returns>Returns a single entity by a single id from table T.</returns>
        public static T GetGuid<T>(this IDbConnection connection, string guid,int tenantId, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            var currenttype = typeof(T);
            var idProps = GetIdProperties(currenttype).ToList();
            ...
            BuildSelect(sb, GetScaffoldableProperties((T)Activator.CreateInstance(typeof(T))).ToArray());
            sb.AppendFormat(" from {0}", name);
            sb.Append(" where guid= @guid and TenantId=@tenantId");

            var dynParms = new DynamicParameters();
            dynParms.Add("@guid", guid);
            dynParms.Add("@tenantId", tenantId);

            if (Debugger.IsAttached)
                Trace.WriteLine(String.Format("Get<{0}>: {1} with Id: {2}", currenttype, sb, guid));
        ...
        public static IEnumerable<T> GetQuery<T>(this IDbConnection connection, string query, object parameters = null, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            var currenttype = typeof(T);
            var idProps = GetIdProperties(currenttype).ToList();
            if (!idProps.Any())
