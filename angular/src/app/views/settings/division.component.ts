...
export class DivisionComponent  implements OnInit{
  ...
  ngOnInit() {
    ...
    //this.service.get('division/list').subscribe(data => {
  ...
  public datasource = (params: any): Observable<any> => {    
    return this.httpService.post(`/division/list`,params);
  };
  ...
  onDelete(obj: any) {
    if (confirm("Are you sure delete this record?")) {
      this.httpService.delete("/division/delete")
        .subscribe(response => {
          this.table.refresh()
  ...
  onEdit(e:any) {
    this.router.navigate(['/settings/division/editing/'+e.id]);
  }
  onCreate() {
    this.router.navigate(['/settings/division/create']);
  }
