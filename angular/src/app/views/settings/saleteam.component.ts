...
@Component({
    selector: 'app-saleteam-component',
     template: `
     <div class="row">
      <div class="col-lg-12">
        <div class="card">
          <div class="card-header">
            <h5 class="mb-0">Sales Teams List</h5>
...
export class SaleTeamComponent  implements OnInit{
  ...
  public datasource = (params: any): Observable<any> => {    
    return this.httpService.post(`/salesteams/list`,params);
  };
  ...
  onDelete(obj: any) {
    if (confirm("Are you sure delete this record?")) {
      this.httpService.get("/salesteams/delete/"+obj.id)
        .subscribe({
          next: () => {
            this.table.refresh()
  ...
  onEdit(e:any) {
    this.router.navigate(['/settings/sales-teams/editing/'+e.id]);
  }
  onCreate() {
    this.router.navigate(['/settings/sales-teams/create']);
  }
 
  public columns: Array<NgTableColumn> = [  
   
    { title: 'Code', name: 'teamCode', sort: true },
    { title: 'Name', name: 'teamName', sort: true },
