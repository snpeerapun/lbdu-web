...
export class BalanceSheetComponent  implements OnInit{
  ...
  onDownload(obj: any) {
    this.httpService.download("/balancesheet/download/" + obj.id).subscribe({
      next: (response: any) => {
  ...
  public datasource = (params: any): Observable<any> => {    
    return this.httpService.post(`/balancesheettemplate/list`,params);
  };
  ...
  onDelete(obj: any) {
    if (confirm("Are you sure delete this record?")) {
      this.httpService.get("/balancesheettemplate/delete/"+obj.id)
        .subscribe(response => {
          this.table.refresh()
  ...
  onEdit(e:any) {
    this.router.navigate(['/budget/balance-sheet/edit/'+e.id]);
  }
  onCreate() {
    this.router.navigate(['/budget/balance-sheet/create']);
  }
 
  public columns: Array<NgTableColumn> = [  
    { title: 'Template Name', name: 'templateName', sort: true },
    { title: 'Description', name: 'description', sort: true },
    { title :'Active', name: 'isActive',template: 'status' },
