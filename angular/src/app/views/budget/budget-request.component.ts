...
@Component({
    selector: 'app-budget-request-component',
     template: `
     <div class="row">
      <div class="col-lg-12">
        <div class="card">
          <div class="card-header">
            <h5 class="mb-0">Budget Request List</h5>
...
export class BudgetRequestComponent  implements OnInit{
  ...
  ngOnInit() {
    ...
    //this.service.get('budgetrequest/list').subscribe(data => {
  ...
  public datasource = (params: any): Observable<any> => {    
    return this.httpService.post(`/budgetrequest/list`,params);
  };
  ...
  async onDelete(item: any) {
    ...
    if (result) {
      this.httpService.get("/budgetrequest/delete/"+item.id)
        .subscribe(response => {
          this.table.refresh()
  ...
  onEdit(e:any) {
    this.router.navigate(['/budget/requests/detail/'+e.id]);
  }
  onCreate() {
    this.router.navigate(['/budget/requests/create']);
  }
  ...
  public columns: Array<NgTableColumn> = [  
    ...
    { title: 'RequestBy', name: 'fullName', sort: true },
    { title: 'Amount', name: 'totalAmount', sort: true ,format:NgTableFormat.Number},    
